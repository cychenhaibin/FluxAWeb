        ...
        <TabsList className="inline-flex bg-zinc-100 p-1 rounded-full">
          {services.map((s) => (
            <TabsTrigger
              key={s.slug}
              value={s.slug}
              className="rounded-full px-4 text-[13.5px] font-medium text-zinc-600 data-[state=active]:bg-white data-[state=active]:text-zinc-900 data-[state=active]:shadow-stripe"
            >
              {s.name}
            </TabsTrigger>
          ))}
